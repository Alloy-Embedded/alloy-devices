...
# embassy core names (cm0p/cm4/cm7/cm33...) -> (our core name, arch)
_ARCH = {"cm0p": ("cm0plus", "armv6m"), "cm0": ("cm0", "armv6m"),
         "cm3": ("cm3", "armv7m"), "cm4": ("cm4", "armv7em"),
         "cm7": ("cm7", "armv7em"), "cm33": ("cm33", "armv8m_main")}
...
def build_chip(sha: str, part: str, family: str, ip_map: dict[str, str],
    ...
    if unmapped:
        raise SystemExit(f"{part}: unmapped ip tags {sorted(unmapped)} — "
                         "add them to builders/st/ip_map.yaml")

    _enrich_family(family, periphs, core, irq_names)

    memories = []
    for m in src["memory"][0] if isinstance(src["memory"][0], list) else src["memory"]:
        kind = m["kind"].lower()
        if kind not in ("flash", "ram"):
            continue
        memories.append({"name": m["name"].lower().replace("bank_1", "flash").replace("bank1", "flash"),
                         "kind": "flash" if kind == "flash" else "ram",
                         "base": f"0x{m['address']:08X}", "size": m["size"]})
    # normalize: single flash + first ram named sram
    flat = []
    seen_flash = seen_ram = False
    for m in memories:
        if m["kind"] == "flash" and not seen_flash:
            m["name"] = "flash"
            seen_flash = True
            flat.append(m)
        elif m["kind"] == "ram" and not seen_ram:
            m["name"] = "sram"
            seen_ram = True
            flat.append(m)

    core_name, arch = _ARCH[core["name"].lower()]
    chip = {
        ...
        "provenance": {
            "sources": [
                ...
                "clock profiles from builders/st/clock_templates/stm32g0.yaml "
                "(boot=HSI16 safe; pll_64mhz hardware-verified recipe, old-alloy 6d47f2cb)",
            ],
        },
        "cores": [{"name": core_name, "arch": arch, "fpu": False,
                   "nvic_prio_bits": _NVIC_PRIO_BITS.get(family, 2)}],
        "memories": flat,
...
def main() -> None:
    ...
    rcc_key = "rcc_g0x1"  # family-specific; extend per family
    rcc_ir = json.loads(_fetch(sha, f"data/registers/{rcc_key}.json"))
    ours = yaml.safe_load((REPO / "registers" / "st" / "rcc_g0.yaml").read_text())
    ...
    wrote = compared = skipped = 0
    for part in parts:
        chip = build_chip(sha, part, args.family, ip_map, overrides, clock,
                          rcc_ir, ours_by_offset)
